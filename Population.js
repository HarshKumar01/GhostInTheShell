class Population {
  ...
  update() {
    for (var i = 0; i < this.players.length; i++) {
      if (this.players[i].brain.step > this.minStep) {
        //if the player has already taken more steps than the best player has taken to reach the goal
  ...
  naturalSelection() {
    ...
    newPlayers[0] = this.players[this.bestPlayer].reproduce();
  ...
  setBestPlayer() {
    ...
    if (max > this.bestFitness) {
      ...
      this.genPlayers.push(this.players[this.bestPlayer].reproduce());
    }
